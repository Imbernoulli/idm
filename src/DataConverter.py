...
def process_json_data(data, total_frames, fps = 30):
    ...
    for item in data:
        if item['type'] == 'drag':
            ...
            new_data.append({"frame": frame, "type": "drag_start", "position": item["start_position"]})
            new_data.append({"frame": frame, "type": "drag_end", "position": item["end_position"]})
            ...
            for frame in range(start_frame, end_frame + 1):
                new_data.append({"frame": frame, "type": "scroll", "position": item["position"], "amount": amount_per_frame})
    ...
    frames_data = [{'frame': i, 'type': None, 'position': None} for i in range(total_frames)]
