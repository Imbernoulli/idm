...
class MixedLoss_1d(nn.Module):
    ...
    def forward(self, outputs, labels):
        ...
        mask = (
            (cursor_true != -1).all(dim=1).float().unsqueeze(1)
        )  # Ensure mask dimension matches loss calculation

        # Cursor position loss calculation
        cursor_loss = self.mse_loss(cursor_pred, cursor_true)
        cursor_loss = (
            cursor_loss * mask
        ).mean()  # Calculate loss only for valid cursor positions
...
class MixedLoss_nd(nn.Module):
    def __init__(self, alpha=0.5):
        ...
        self.ce_loss = nn.CrossEntropyLoss(
            reduction="none"
        )  # 设置 reduction='none' 来手动控制损失的计算

    def forward(self, outputs, labels):
        num_images = outputs.shape[0]  # 获取一次传入图片的数量

        cursor_pred = outputs[:, :, :2].view(
            -1, 2
        )  # 将光标位置预测值重塑为 (num_images * num_samples_per_image, 2)
        action_pred = outputs[:, :, 2:].view(
            -1, outputs.shape[-1] - 2
        )  # 将动作类型预测值重塑为 (num_images * num_samples_per_image, num_actions)

        cursor_true = labels[:, :, :2].view(
            -1, 2
        )  # 将光标位置真实值重塑为 (num_images * num_samples_per_image, 2)
        action_true = labels[:, :, 2:].view(
            -1, labels.shape[-1] - 2
        )  # 将动作类型真实值重塑为 (num_images * num_samples_per_image, num_actions)
